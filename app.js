...
// Автоматическое отключение тёмной темы и перенос схемы В КОНЕЦ документа перед печатью
window.addEventListener('beforeprint', function () {
    ...
    if (scheme && printArea) {
        // Переносим схему в абсолютный конец документа (после Итого и подвала)
        printArea.appendChild(scheme);
    }
});
...
window.addEventListener('afterprint', function () {
    ...
    let scheme = document.getElementById('dynamic_scheme');
    let tableWrapper = document.querySelector('.table-responsive');
    if (scheme && tableWrapper) {
        // Возвращаем схему на её законное место над таблицей
        tableWrapper.insertAdjacentElement('beforebegin', scheme);
    }
});