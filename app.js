...
const app = {
    ...
    syncUI: function () {
        ...
        if (document.getElementById('chk_cheaper')) {
            document.getElementById('chk_cheaper').checked = (this.state.brandMode === 'rommer');
        }
        // Обновляем бейдж с процентом экономии
    ...
    render: function () {
        ...
        const addToBill = (item, qty, tip, group = null) => {
            ...
            if (this.state.brandMode === 'rommer' && item.rommer) {
                ...
                // Если у роммера есть свои аналоги (alts), заменяем и их
                if (item.rommer.alts) finalItem.alts = item.rommer.alts;
            }

            this.currentSpec.push({ ...finalItem, q: qty, group: group });

            // Считаем разницу для бейджа скидки
            this.calcBaseTotal += (item.price || 0) * qty;
            this.calcFinalTotal += (finalItem.price || 0) * qty;
            ...
            if (forceMerge) {
                ...
                if (existing) {
                    ...
                    bill.push({ ...finalItem, q: qty, sum: finalItem.price * qty, displaySku: finalItem.id, qtyTip: finalTip || "", group: group, originalId: finalItem.id });
                }
            } else {
                let finalTip = tip;
