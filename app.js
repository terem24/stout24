...
const app = {
    ...
    // ====================================
    render: function () {
        // Динамическое назначение списков замен (alts) перед каждым рендером
        ...
        const radAlts = [catalog.rads[0], titanRads[0], steelRads[0]];
        
        catalog.rads.forEach(rad => { rad.alts = radAlts; if (rad.rommer) rad.rommer.alts = radAlts; }); 
        titanRads.forEach(rad => { rad.alts = radAlts; if (rad.rommer) rad.rommer.alts = radAlts; }); 
        steelRads.forEach(rad => { rad.alts = radAlts; if (rad.rommer) rad.rommer.alts = radAlts; });
        
        let hAlts = catalog.h_valves; catalog.h_valves.forEach(v => { v.alts = hAlts; if (v.rommer) v.rommer.alts = hAlts; });
        let boilerAlts = [catalog.tanks_optibase[0], catalog.tanks_standard[0]]; 
        catalog.tanks_optibase.forEach(t => { t.alts = boilerAlts; if (t.rommer) t.rommer.alts = boilerAlts; }); 
        catalog.tanks_standard.forEach(t => { t.alts = boilerAlts; if (t.rommer) t.rommer.alts = boilerAlts; });
        
        let pumpAlts = catalog.pumps_dn25; catalog.pumps_dn25.forEach(p => { p.alts = pumpAlts; if (p.rommer) p.rommer.alts = pumpAlts; });
        let elBoilerAlts = [catalog.boilers_status[0], catalog.boilers_plus[0]]; 
        catalog.boilers_plus.forEach(b => { b.alts = elBoilerAlts; if (b.rommer) b.rommer.alts = elBoilerAlts; }); 
        catalog.boilers_status.forEach(b => { b.alts = elBoilerAlts; if (b.rommer) b.rommer.alts = elBoilerAlts; });
        
        let hydroAlts = catalog.hydro_modular_dn20; 
        catalog.hydro_dn20.forEach(h => { h.alts = hydroAlts; if (h.rommer) h.rommer.alts = hydroAlts; }); 
        catalog.hydro_modular_dn20.forEach(h => { h.alts = catalog.hydro_dn20; if (h.rommer) h.rommer.alts = catalog.hydro_dn20; });
        
        let pipeAlts = catalog.rad_pipes_grey; 
        catalog.insulated_pipes.forEach(p => { p.alts = pipeAlts; if (p.rommer) p.rommer.alts = pipeAlts; }); 
        catalog.rad_pipes_grey.forEach(p => { p.alts = catalog.insulated_pipes; if (p.rommer) p.rommer.alts = catalog.insulated_pipes; });
        
        if (catalog.manifolds_rad && catalog.manifolds_chrome_blocks) { 
            let chromeAlt = catalog.manifolds_chrome_blocks[0]; 
            catalog.manifolds_rad.forEach(m => { m.alts = [chromeAlt]; if (m.rommer) m.rommer.alts = [chromeAlt]; }); 
            catalog.manifolds_chrome_blocks.forEach(m => { m.alts = [catalog.manifolds_rad[0]]; if (m.rommer) m.rommer.alts = [catalog.manifolds_rad[0]]; }); 
        }
        let xpsAlt = catalog.xps_kit[0]; catalog.mats.forEach(m => { m.alts = [xpsAlt]; if (m.rommer) m.rommer.alts = [xpsAlt]; }); 
        catalog.xps_kit[0].alts = catalog.mats;
        if (catalog.well_auto) { let waAlts = catalog.well_auto; catalog.well_auto.forEach(a => { a.alts = waAlts; if (a.rommer) a.rommer.alts = waAlts; }); }
