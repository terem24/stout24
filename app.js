...
const app = {
    ...
    showProForm: function (planName) {
        ...
        container.innerHTML = `
            <div class="modal-subscription" style="text-align: center; padding: 30px; background: #fff; border-radius: 12px; max-width: 400px; margin: auto;">
                <h2 style="font-size: 24px; font-weight: 700; margin-bottom: 25px; color: #000;">Подписка PRO</h2>
                ...
                <input type="text" id="sub_name" placeholder="Имя и фамилия" value="${user.name || user.first_name || ''}" 
                    style="width: 100%; height: 50px; border: 1px solid #ddd; border-radius: 8px; padding: 0 15px; margin-bottom: 12px; font-size: 16px;">
                    
                <input type="email" id="sub_email" placeholder="Email" value="${user.email || ''}" 
                    style="width: 100%; height: 50px; border: 1px solid #ddd; border-radius: 8px; padding: 0 15px; margin-bottom: 12px; font-size: 16px;">
                    
                <input type="tel" id="sub_phone" placeholder="Телефон" value="${user.phone || ''}" 
                    style="width: 100%; height: 50px; border: 1px solid #ddd; border-radius: 8px; padding: 0 15px; margin-bottom: 20px; font-size: 16px;">
                    
                <button onclick="app.sendSubscriptionRequest()" id="btn_pro_submit" 
                    style="width: 100%; height: 55px; background: #00A859; color: #fff; border: none; border-radius: 6px; font-size: 18px; font-weight: 600; cursor: pointer;">
                    Отправить заявку
                </button>
                
                <a href="#" onclick="app.closeModal()" style="display: block; margin-top: 15px; color: #999; text-decoration: none; font-size: 14px;">Закрыть</a>
            </div>
        `;
    ...
    sendEmail: async function () {
        ...
        try {
            ...
            const templateParams = {
                ...
                user_city: (tgUser.city || this.state.user?.city || 'Не указан'),
