    ...
    render: function () {
        ...
        if (this.state.userAddedEq && this.state.userAddedEq.length > 0) {
            this.state.userAddedEq.forEach(eq => {
                // Путь к сохраненной иконке (unnamed.jpg)
                let iconSrc = 'unnamed.jpg'; 
                
                // HTML-код иконки с выравниванием по центру строки текста
                let iconHtml = `<img src="${iconSrc}" alt="" style="height: 1.2em; width: auto; vertical-align: middle; margin-right: 6px; position: relative; top: -1px; border-radius: 2px;" />`;

                // Встраиваем крестик для удаления И иконку прямо в название
                let customName = `<span class="work-del-btn" style="color:#EF4444; font-weight:bold; cursor:pointer; margin-right:8px; font-size:14px;" onclick="event.stopPropagation(); app.deleteEq('${eq.id}')" title="Удалить">✖</span>${iconHtml}${eq.name}`;
                
                // Создаем временный объект товара для спецификации
                let customEqItem = { ...eq, name: customName };

                // Добавляем в смету. Третий аргумент (бренд) устанавливаем в ПУСТУЮ СТРОКУ "".
                addToBill(customEqItem, eq.q, "", "9. Своё оборудование");
            });
            // Финализируем группу в смете
