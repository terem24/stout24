...
const app = {
    ...
    render: function () {
        ...
        if (this.state.hotWater) {
            ...
            // Безопасный цикл подбора бойлера
            let searchVol = targetVol;
            let tank = null;
            let tankDb = (this.state.boilerType === 'optibase') ? catalog.tanks_optibase : catalog.tanks_standard;
            let boilerWarn = "";
            
            while (!tank) {
                // Ищем бойлер, который больше или равен нужному объему
                tank = tankDb.find(t => t.vol >= searchVol); 
                
                if (!tank) {
                    searchVol += 10; // Шаг поиска
                    
                    // ПРЕДОХРАНИТЕЛЬ: Жесткий выход при превышении лимита
                    if (searchVol > 500) {
                        // Назначаем самый большой доступный бойлер (500л), чтобы скрипт мог продолжить работу
                        tank = tankDb.find(t => t.vol === 500) || tankDb[tankDb.length - 1];
            ...
            // Если предупреждение не пустое, добавим его и в общий список предупреждений (если он есть)
            if (boilerWarn && app.tempWarns) app.tempWarns.push(boilerWarn);
        }
