    ...
    render: function () {
        ...
        if (hasRad && radSecs > 0) {
            ...
            if (totalRadCount > 0) {
                ...
                addToBill(catalog.parts[1], totalRadCount * 2, "Евроконус 16 (Рад).", grp); addToBill(catalog.parts[2], totalRadCount * 2, "Фиксатор 90°.", grp);
            }
            if (totalConvCount > 0) {
                let grpC = "3.2. Обвязка конвекторов";
                let isAngled = (this.state.convConnectionType === 'angled');
                addToBill(isAngled ? catalog.conv_valves[2] : catalog.conv_valves[0], totalConvCount, "На подачу.", grpC);
                addToBill(isAngled ? catalog.conv_valves[3] : catalog.conv_valves[1], totalConvCount, "На обратку.", grpC);
                if (this.state.convectorType === 'scq') {
                    addToBill(catalog.actuators, totalConvCount, "Сервопривод.", grpC);
                    addToBill(catalog.conv_parts[2], totalVartronic || 1, "Регулятор Vartronic.", grpC);
                }
            }
        }
        flushBill("3. Приборы отопления");

        // === 5. ВНУТРЕННЕЕ ВОДОСНАБЖЕНИЕ ===
        if (this.state.water) {
            let grpW = "5. Внутреннее водоснабжение";
            let totalFeedPoints = 0;
            this.state.waterZones.forEach(z => {
                let p = (z.fixtures.toilet || 0) + (z.fixtures.basin || 0) + (z.fixtures.shower || 0) + (z.fixtures.bath || 0) + (z.fixtures.wash || 0) + (z.fixtures.dish || 0);
                totalFeedPoints += p;
            });
            addToBill(catalog.water_parts[0], Math.ceil(totalFeedPoints / 2), "Труба PEX-a 16x2.2 (ХВС/ГВС).", grpW);
            addToBill(catalog.water_parts[1], totalFeedPoints * 2, "Водорозетка.", grpW);
            addToBill(catalog.parts[1], totalFeedPoints * 2, "Евроконус 16 (Вода).", grpW);
            flushBill(grpW);
        }

        // === 6. УЗЕЛ ВВОДА ХВС ===
        if (this.state.waterInput) {
            let grpInput = "6. Узел ввода ХВС";
            catalog.water_input_node.forEach(item => {
                addToBill(item, (item.id === "SFT-0004-003434" ? 2 : 1), "Комплектация узла ввода.", grpInput);
            });
            flushBill(grpInput);
            addToWorks("Монтаж узла ввода ХВС (сборка, насос, фильтрация, греющий кабель)", 1, 20000, "компл", "1.1 Монтаж котельной");
        }

        // === 7. ВНЕШНЕЕ ВОДОСНАБЖЕНИЕ ===
        if (this.state.well) {
            ...
            if (q > 4.5) q = 4.5 + (q - 4.5) * 0.5; if (q < 1.5) q = 1.5;
            let h_calc = (parseInt(this.state.wellDepth) + (parseInt(this.state.wellDist) / 10) + (this.state.floors === 2 ? 3 : 0) + 30) * 1.1;
            let pump = catalog.well_pumps.find(p => p.q_max >= (q * 0.9) && p.h_max >= (h_calc + 20)) || catalog.well_pumps[catalog.well_pumps.length - 1];
            addToBill(pump, 1, `Насос для скважины (Расчет: ${q.toFixed(1)} м3/ч, Напор ${Math.round(h_calc)}м).`, grpWell);
            addToWorks("Монтаж скважинного насоса (опуск, оголовок, автоматика)", 1, 15000, "компл", "2.1 Внешнее водоснабжение");
            addToWorks("Прокладка трубы ПНД в траншее", parseInt(this.state.wellDist), 400, "м.p.", "2.1 Внешнее водоснабжение");
            flushBill(grpWell);
        }

        // === 8. КАНАЛИЗАЦИЯ ===
        let totalToilets = this.state.waterZones.reduce((sum, z) => sum + (z.fixtures.toilet || 0), 0);
        if (totalToilets > 0) {
            addToBill(catalog.water_parts[6], totalToilets, "Инсталляция унитаза.", "8. Канализация");
            addToWorks("Монтаж инсталляции унитаза", totalToilets, 8000, "шт", "3.1 Внутренняя канализация");
        ...
        if (cl) {
            let tpArea = this.state.area || 0;
            let vFinal = (totalRadCount * 0.25 + totalConvCount * 1.5 * 0.11 + tpArea * 7 * 0.113 + 10) * 1.15;
            addToBill(cl, Math.ceil(vFinal / cl.vol), `Теплоноситель, объем ~${Math.round(vFinal)}л.`, "9. Дополнительные материалы");
        }
        flushBill("9. Дополнительные материалы");

        // === 10. СВОЁ ОБОРУДОВАНИЕ ===
        if (this.state.viewMode === 'equipment' && this.state.userAddedEq) {
            this.state.userAddedEq.forEach(eq => {
                addToBill({ ...eq, brand: eq.brand || " " }, eq.q, eq.desc || "", "10. Своё оборудование");
            });
            h += `<tr class="no-print"><td colspan="9" style="padding:15px; text-align:center;"><div onclick="app.addCustomEqPrompt()" class="btn-add-custom">+ Добавить оборудование</div></td></tr>`;
            flushBill("10. Своё оборудование");
        }

        // === WORKS SECTION ===
        let wGroup = "1.1 Монтаж котельной";
        if (this.state.fuels.includes('el')) addToWorks("Mонтаж электрического котла", 1, 18000, "шт", wGroup);
        if (this.state.fuels.includes('gas')) {
            addToWorks("Монтаж газового котла", 1, 20000, "шт", wGroup);
            addToWorks("Монтаж коаксиального дымохода", 1, 10000, "шт", wGroup);
        }
        if (this.state.hotWater) {
            addToWorks("Монтаж бойлера", 1, 9000, "шт", wGroup);
            addToWorks("Подключение бойлера", 1, 12000, "компл", wGroup);
        }
        if (this.state.systems.includes('tp') && tpArea > 0) {
            let tpG = "1.3 Монтаж водяного теплого пола";
            addToWorks("Монтаж труб ТП", tpArea, 450, "м2", tpG);
            addToWorks("Монтаж утеплителя ТП", tpArea, 390, "м2", tpG);
        }
        if (this.state.userAddedWorks) {
            this.state.userAddedWorks.forEach(w => { addToWorks(w.name, w.q, w.price, w.unit, w.group); });
        }
        flushWorks();

        // --- FINAL RENDERING & WRAP-UP ---
        document.getElementById('tbody').innerHTML = h;
        document.getElementById('total_sum').innerText = sum.toLocaleString() + " ₽";
        let d_sku = showSku ? 'table-cell' : 'none';
        document.querySelectorAll('.col-sku').forEach(e => e.style.display = d_sku);
        document.querySelector('.col-sku-head').style.display = d_sku;
        ...
        if (headerTotals) {
            ...
            if (!headerTotals.innerHTML.includes('anim_eq_sum')) {
                let htmlStr = `<span style="color:var(--text-sec); font-size:11px; margin-right:4px;">Оборудование:</span> <b id="anim_eq_sum" style="color:var(--primary); font-size:14px;">0 ₽</b>`;
                if (isPro) htmlStr += `<span style="margin:0 10px; color:var(--border);">|</span> <span style="color:var(--text-sec); font-size:11px; margin-right:4px;">Монтаж:</span> <b id="anim_works_sum" style="color:#F97316; font-size:14px;">0 ₽</b>`;
                headerTotals.innerHTML = htmlStr;
                headerTotals.dataset.lastEq = 0; headerTotals.dataset.lastWorks = 0;
            }
            let elEq = document.getElementById('anim_eq_sum');
            let oldEq = parseFloat(headerTotals.dataset.lastEq) || 0;
            let newEq = app.lastEqSum || 0;
            if (oldEq !== newEq && elEq) { app.animateNumber(elEq, oldEq, newEq, 2400); headerTotals.dataset.lastEq = newEq; }
            if (isPro) {
                ...
                if (oldWorks !== newWorks && elWorks) { app.animateNumber(elWorks, oldWorks, newWorks, 2400); headerTotals.dataset.lastWorks = newWorks; }
            }
        }

        let oldScheme = document.getElementById('dynamic_scheme'); if (oldScheme) oldScheme.remove();
        if (this.state.viewMode === 'equipment' && this.state.showScheme) {
            let tableWrapper = document.querySelector('.table-responsive');
            if (tableWrapper) tableWrapper.insertAdjacentHTML('beforebegin', this.renderScheme());
        }

        this.saveState();
        if (this.isAppReady) {
            let isDifferent = (this.lastSavedStateString !== this.getStateSignature());
            if (isDifferent) this.markAsUnsaved(); else this.markAsSaved();
        }

        let dBadge = document.getElementById('discount_badge');
        if (dBadge) {
            if (this.state.brandMode === 'rommer' && this.calcBaseTotal > this.calcFinalTotal) {
                let percent = Math.round(((this.calcBaseTotal - this.calcFinalTotal) / this.calcBaseTotal) * 100);
                dBadge.textContent = 'Экономия ' + percent + '%'; dBadge.style.display = 'block';
...
window.addEventListener('beforeprint', () => {
    ...
    let printBin = document.getElementById('print_bin') || document.createElement('div');
    printBin.id = 'print_bin'; if (!document.getElementById('print_bin')) document.body.appendChild(printBin);
    printBin.innerHTML = '';
    let originalMode = app.state.viewMode;
    let printArea = document.getElementById('print-area');
    if (printArea) {
        app.state.viewMode = 'equipment'; app.render();
        let eqClone = printArea.cloneNode(true);
        eqClone.querySelectorAll('.main-view-tabs, #dynamic_scheme').forEach(el => el.remove());
        ...
        if (isPro) {
            app.state.viewMode = 'works'; app.render();
            let worksClone = printArea.cloneNode(true);
            worksClone.classList.add('print-page-break');
            worksClone.querySelectorAll('.main-view-tabs, #dynamic_scheme').forEach(el => el.remove());
            printBin.appendChild(worksClone);
        }
        printArea.classList.add('hide-original-for-print');
        app.state.viewMode = originalMode; app.render();
    }
});

window.addEventListener('afterprint', () => {
    if (app.state && app.state.darkMode) document.body.classList.add('dark-mode');
    document.getElementById('print-area')?.classList.remove('hide-original-for-print');
    let printBin = document.getElementById('print_bin'); if (printBin) printBin.innerHTML = '';
});
...
window.addEventListener('scroll', () => {
    ...
    if (header) {
        ...
        // ==========================================
        radGroup = "1.2 Монтаж радиаторного отопления";
        ...
        // ==========================================
        tpGroup = "1.3 Монтаж водяного теплого пола";
        ...
        // ==========================================
        autoGroup = "1.4 Автоматика для теплого пола";
        ...
        if (headerTotals) {
            ...
            if (!headerTotals.innerHTML.includes('anim_eq_sum') || headerTotals.dataset.isPro !== String(isPro)) {
                let html = `<span style="color:var(--text-sec); font-size:12px; margin-right:5px;">Оборудование:</span> <b id="anim_eq_sum" style="color:#2563EB; font-size:15px;">0 ₽</b>`;
                if (isPro) {
                    html += `<span style="margin:0 12px; color:#E5E7EB;">|</span> <span style="color:var(--text-sec); font-size:12px; margin-right:5px;">Монтаж:</span> <b id="anim_works_sum" style="color:#F97316; font-size:15px;">0 ₽</b>`;
                }
