    ...
    filterAdminData: function (query) {
        ...
        rows.forEach(row => {
            ...
            renderScheme: function() {
                ...
                // 1. Базовые слои (всегда видны)
                // Сначала фон-рамка, затем расширительный бак отопления
                layers.push('bg_frame.png');
                layers.push('tank_heating.png');

                // 2. Блок Котлов (Логика 'выбор + патрубки + общая труба')
                const hasGas = s.fuels.includes('gas');
                const hasEl = s.fuels.includes('el');
                ...
                // Общая магистраль (подача/обратка) добавляется, если выбран любой котел
                if (hasGas || hasEl) {
                ...
                if (s.hotWater === true) {
                    ...
                    layers.push('bkn_connection.png');
                    layers.push('tank_water.png');
                }

                // 4. Блок Распределения и Потребителей
                // Гидрострелка: если суммарно выбрано 2 и более контура (системы + ГВС)
                const circuitsCount = s.systems.length + (s.hotWater ? 1 : 0);
                if (circuitsCount >= 2) {
                ...
                // Генерация HTML
                // Responsive высота 70vh с ограничениями, чтобы схема не требовала скролла
                ...
                layers.forEach(layer => {
                    // Все слои накладываются друг на друга и вписываются в контейнер через object-fit
                    html += `<img src="${basePath}${layer}" alt="${layer}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: contain;">`;
                });
